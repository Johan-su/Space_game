...
namespace Ecs
{
    ...
    void set_min_array_data(Component_data *cdata, Usize *compids, Usize *min_id, Usize *min_count)
    {
        ...
        *min_count = SIZE_MAX;
        ...
        ([&]()
        {
            Usize compid = get_component_id<T>(cdata);
...
namespace Ecs
{
    ...
    {
        init_component_bytes(registry->mm, &registry->cdata, get_component_id<T>(&registry->cdata), sizeof(T), alignof(T));
    }
    ...
    {
        set_component_raw(registry->mm, &registry->cdata, e, (void *)&comp, get_component_id<T>(&registry->cdata), sizeof(T));
    }
    ...
    {
        return (T *)get_component_raw(registry->mm, &registry->cdata, e, get_component_id<T>(&registry->cdata), sizeof(T));
    }


    template<typename T>
    void destroy_component(Registry *registry, Entity e)
    {
    ...
    View<T1> *get_view(Registry *registry)
    {
        ...
        Usize mincompid;
        Usize minsize;
        set_min_array_data<T1, Ts...>(&registry->cdata, compids, &mincompid, &minsize);


        //TODO(Johan): allocation with minsize might waste a lot of memory.
        View<T1> *view = Arena::top_alloc<View<T1>>(registry->view_mm); 
        view->entity_list = Arena::top_alloc<Entity>(registry->view_mm, minsize);
        view->comparray = Arena::top_alloc<T1>(registry->view_mm, minsize);
        view->size = 0;
        ...
        Component_pool<T1> *comp_pool = get_component_pool<T1>(&registry->cdata);

        fill_similar_entities(&registry->cdata, view->entity_list, &view->size, compids, mincompid, typeCount);
    ...
    const Group *get_group(Registry *registry)
    {
        ...
        set_min_array_data<T...>(&registry->cdata, compids, &mincompid, &min_count);


        Group *group = Arena::top_alloc<Group>(registry->view_mm);

        group->comp_arrays     = Arena::top_alloc<void *>(registry->view_mm, typeCount);
        group->comp_sizes      = Arena::top_alloc<Usize>(registry->view_mm, typeCount);
        group->comp_alignments = Arena::top_alloc<Usize>(registry->view_mm, typeCount);
        group->entity_list     = Arena::top_alloc<Entity>(registry->view_mm, min_count);

        group->size = 0;

        for (Usize i = 0; i < typeCount; ++i)
        {
            Usize comp_alignment = registry->cdata.component_alignments[compids[i]];
            Usize comp_size      = registry->cdata.component_sizes[compids[i]];

            group->comp_alignments[i] = comp_alignment;
            group->comp_sizes[i]      = comp_size;

            group->comp_arrays[i] = Arena::top_alloc_bytes(registry->view_mm,  min_count * comp_size, comp_alignment);
        }
        ...
        fill_similar_entities(&registry->cdata, group->entity_list, &group->size, compids, mincompid, typeCount);
        ...
        for (Usize i = 0; i < group->size; ++i)
        {
            ...
            {
                Usize comp_size = registry->cdata.component_sizes[compids[j]];


                // not actually a pool of U8:s, it is used to read individual bytes.
                Component_pool<U8> *comp_pool = (Component_pool<U8> *)registry->cdata.component_pools[compids[j]];
